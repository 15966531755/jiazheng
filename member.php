...
if($a=='saveedit'    or $a=='getarea'    or 
   ...
   $a=='removeoqq'   or $a=='removeoweibo' or $a=='removeoweixin')
{
	if(!empty($c_uname))
	{
...

{
	...
	if($cfg_oauth == 'Y' && isset($method) && $method == 'callback')
	{
		...
		//检测账号登录状态
		if(check_app_login('qq') or check_app_login('weibo') or check_app_login('weixin'))
		{
			...
			if(check_app_login('qq'))
				...
				else if(check_app_login('weixin'))
			
				$sql = "SELECT * FROM `#@__member` WHERE `weixinid`='".$_SESSION['app']['weixin']['uid']."'";
			//echo $sql;
//			exit();
...
if(check_app_login('qq'))
              $linshi = $_SESSION['app']['qq']['name'];
			else if(check_app_login('weibo'))
			  
			   $linshi = $_SESSION['app']['weibo']['name'];
			 
			 	else if(check_app_login('weixin'))
			  
			   $linshi = $_SESSION['app']['weixin']['name'];
	...
	{
 // if(isset($_POST['gourl'])){
//    $gourls = $_POST['gourl'] ? $gourls : '';
//	$gourl = $_POST['gourl'] ? '&url='.$gourl : '';
  //}
		//初始化参数
...
//echo  $username."<br>1";
//echo  $_SESSION['mobile']."<br>2";
//echo  $mobile_code."<br>3";
//echo  $_SESSION['mobile_code']."<br>4";

	//echo '<pre>';print_r($_POST);print_r($_SESSION);
	if($username!=$_SESSION['mobile'] or $mobile_code!=$_SESSION['mobile_code'] or empty($username) or empty($mobile_code)){
		ShowMsg('手机验证码输入错误！','?c=reg');
		exit();
	}else{
		$_SESSION['mobile'] = '';
...
//	}
	...
	$sql = "INSERT INTO `#@__member` (username, password,  expval, regtime, regip, logintime, loginip) VALUES ('$username', '$password',  '10', '$regtime', '$regip', '$regtime', '$regip')";	
...
else if($a == 'manager')
{
	...
	$r = $dosql->GetOne("SELECT `id` FROM `#@__admin` WHERE `username`='$username'");
	if(isset($r['id']))
	{
		ShowMsg('用户名已存在！','?c=manager');
		exit();
	}
...
//	}
	...
	$sql = "INSERT INTO `#@__admin` (username, password, nickname,  levelname, checkadmin, loginip, logintime, card, province, city, area) VALUES ('$username', '$password', '$t_name', '3', 'false', '$regip', '$regtime', '$card', '$province', '$city', '$area')";	
	if($dosql->ExecNoneQuery($sql))
	{
		ShowMsg('注册成功！','admin');
		exit();
...
else if($a == 'savefavorite')
{
    ...
    $link = "goodsshow.php?cid=".$cid."&tid=".$tid."&id=".$aid;
...
if(!empty($c_uname)){
	...
	$r2 = $dosql->GetOne("SELECT `aid`,`molds` FROM `#@__userfavorite` WHERE `aid`=$aid and `molds`=$molds and uname='$uname'");
	if(!is_array($r2))
	{
		$dosql->ExecNoneQuery("INSERT INTO `#@__userfavorite` (aid,molds,uid,uname,link,time,ip,isshow,total) VALUES ('$aid','$molds','$uid','$uname','$link','$time','$ip','1','1')");
...
//		$dosql->ExecNoneQuery("UPDATE `#@__member` SET expval='".($r['expval'] + 1)."', integral='".($r['integral'] + 2)."' WHERE `username`='$c_uname'");
	...
	else
	{
		echo '2';
		exit();
	//$dosql->ExecNoneQuery("update `#@__userfavorite` set total=total+1 where aid=$aid and molds=4 and uid=$uid");
//
//		echo '1';
//		exit();
	}
...
//	$ip    = GetIP();
//
//	$r2 = $dosql->GetOne("SELECT `aid`,`molds` FROM `#@__userfavorite` WHERE `aid`=$aid and `molds`=$molds and `uid`=0");
//	if(!is_array($r2))
//	{
//		$dosql->ExecNoneQuery("INSERT INTO `#@__userfavorite` (aid,molds,uid,uname,link,time,ip,isshow,total) VALUES ('$aid','$molds','$uid','$uname','$link','$time','$ip','1','1')");
//
...
//	else
//	{
//		$dosql->ExecNoneQuery("update `#@__userfavorite` set total=total+1 where aid=$aid and molds=4 and uid=0");
//		echo '1';
//		exit();
//	}
//	
...
//		$dosql->ExecNoneQuery("UPDATE `#@__member` SET expval='".($r['expval'] + 1)."', integral='".($r['integral'] + 2)."' WHERE `username`='$c_uname'");
	...
	{
			//$dosql->ExecNoneQuery("update `#@__userfavorite` set total=total+1 where aid=$aid and molds=44 and uid=$uid");
//		echo '1';
//		exit();
        echo '2';
		exit();
...
//	else
//	{
//		$dosql->ExecNoneQuery("update `#@__userfavorite` set total=total+1 where aid=$aid and molds=44 and uid=0");
//		echo '1';
//		exit();
//	}
//	
...
else if($a == 'delorder')
{
	if(is_array($checkid))
	{
		foreach($checkid as $v)
		{
			...
			//$r = $dosql->GetOne("SELECT `checkinfo` FROM `#@__yuyue` WHERE `id`=$v");
//			$checkinfo = explode(',', $r['checkinfo']);
//			if(in_array('overorder',  $checkinfo))
				$dosql->ExecNoneQuery("DELETE FROM `#@__yuyue` WHERE `id`=$v AND `username`='$c_uname'");
		}
	}
...
{
	$r = $dosql->GetOne("SELECT `checkinfo` FROM `#@__yuyue` WHERE `username`='$c_uname' AND `id`=$id");
	$checkinfo = explode(',',$r['checkinfo']);
	...
	$dosql->ExecNoneQuery("UPDATE `#@__yuyue` SET checkinfo='$checkinfo' WHERE `username`='$c_uname' AND `id`=$id");
...
else if($a == 'perfect')
{
	...
	{
		$r = $dosql->GetOne("SELECT `id` FROM `#@__member` WHERE `weixinid`='".$_SESSION['app']['weixin']['uid']."'");
		if(isset($r['id']))
			ShowMsg('该微信已与其他账号绑定！','-1');
		else
			$sql = "INSERT INTO `#@__member` (username, password, email, expval, regtime, regip, logintime, loginip, weixinid) VALUES ('$username', '$password', '$email', '10', '$regtime', '$regip', '$regtime', '$regip', '".$_SESSION['app']['weixin']['uid']."')";	
	}
...
else if($a == 'binding')
{
	...
	else
	{
		...
		else if(check_app_login('weixin'))
		{
			...
			{
				$weixinid = $_SESSION['app']['weixin']['uid'];
				$sql = "UPDATE `#@__member` SET `weixinid`='$weixinid' WHERE `username`='$username'";
			}
		}
...
//移除绑定微信
else if($a == 'removeoweixin')
{
	$r = $dosql->GetOne("SELECT `id` FROM `#@__member` WHERE `username`='$c_uname' AND `weixinid`<>''");
	if(empty($r) && !is_array($r))
	{
		ShowMsg('错误的操作，您没有绑定微信账号！','-1');
	}
	else
	{
		$dosql->ExecNoneQuery("UPDATE `#@__member` SET `weixinid`='' WHERE `username`='$c_uname'");
		ShowMsg('解除微信绑定成功！','?c=edit');
	}
...
if($c == 'login')
{
		...
		//$name = $_POST['name_x'];
//	$mobile =   isset($mobile_x)   ? $mobile_x   : '';
...
{
	require_once(PHPMYWIND_TEMP.'/default/member/manager.php');
	exit();
}
