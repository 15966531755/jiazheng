...
class Page
{
	...
	function GetList()
	{
		...
		else
		{
			...
			$pagetxt = '<ul class="page_list" style="margin:0px auto; width:480px;">';
			...
			if($this->totalpage < 10)
			{
				for($i=1; $i <= $this->totalpage; $i++)
				{
					if($this->page == $i)
					{
						$pagetxt .= '<li><a href="javascript:;" class="on">'.$i.'</a></li>';
					}
					else
					{
						//伪静态设置
			...
			else
			{
				...
				for($i=$m; $i<=$b; $i++)
				{
					if($this->page == $i)
					{
						$pagetxt .= '<li><a href="'.$nowurl.'page='.$i.'" class="on">'.$i.'</a></li>';
					}
					else
					{
						//伪静态设置
...
//				{
		...
		//获取分页变量
	function GetPage_web($sql,$pagenum=20)
	{
		global $dosql;
