...
if($a == 'login')
{
	...
	$gourl = $_POST['gourl'] ? '&url='.$gourl : '';
		...
		if(is_array($r))
		{
			...
			{
				ShowMsg('您的密码已连续错误6次，请15分钟后再进行登录！','?c=login'.$gourl);
				exit();
			}
		}
		...
		else
		{
			...
			if(!is_array($row) or $password!=$row['password'])
			{
				...
				if(is_array($r))
				{
					...
					{
						$dosql->ExecNoneQuery("UPDATE `#@__failedlogin` SET `time`=$logintime, `num`=$num WHERE `username`='$username'");
						ShowMsg('用户名或密码不正确！您还有'.$num.'次尝试的机会！','?c=login'.$gourl);
						exit();
					}
				}
				else
				{
					...
					ShowMsg('用户名或密码不正确！您还有5次尝试的机会！','?c=login'.$gourl);
					exit();
				}
			}
			...
			{
				ShowMsg('抱歉，您的账号被禁止登陆！','?c=login'.$gourl);
				exit();
			}
			...
			else
			{
				...
				//header('location:./');
				if($gourls != ""){
                    echo '<script>location.href="'.$gourls.'";</script>';
				}else{
					header('location:./');
					}
				exit();
