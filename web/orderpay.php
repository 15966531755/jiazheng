...
$rows = $dosql->GetOne("SELECT * FROM `#@__goods` WHERE  `id`=".$row['goods_id']."");
...
/**************************请求参数**************************/
       ...
       // $subject = $row['username'];
	    $subject = "阳光阿姨 ".$rows['title']." ".$rows['typename']." 服务时间:".$row['date'];
        //必填
        ...
        //服务展示地址
        $show_url = $cfg_weburl.$cfg_webpath.'goodsshow-'.$rows["classid"].'-'.$rows["typeid"].'-'.$rows["id"].'-1.html';
