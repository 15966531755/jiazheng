 ...
 */
function GetHeader($sid=1,$cid=0,$id=0,$str='',$tid=0)
{
	...
	else
	{
		...
		//显示栏目信息
		else if(!empty($cid) and !empty($tid))
		{
			$r = $dosql->GetOne("SELECT * FROM `#@__infoclass` WHERE `id`=$cid");
		$rr = $dosql->GetOne("SELECT * FROM `#@__goodstype` WHERE `id`=$tid");

			$header_str = '<title>';
	
			if(!empty($r['seotitle']))
				$header_str .= $r['seotitle'];
			else if(!empty($r['classname']) and !empty($rr['classname']))
				$header_str .= $rr['classname'].'-'.$r['classname'].' - '.$cfg_webname;
			else
 ...
 * @return  string    返回导航
 */
function GetNavs($pid=1)
{
	global $dosql, $cfg_isreurl;

	$str = '';
	$dosql->Execute("SELECT * FROM `#@__goodstype` WHERE  checkinfo=true ORDER BY orderid ASC");
	while($row = $dosql->GetArray())
	{
		if($cfg_isreurl != 'Y')
			$gourl = "goods.php?cid=21&tid=".$row['id'];
		else
			
			$gourl = "goods-21-".$row['id']."-1.html";
		
		
			$classname = $row['classname'];

		$str .= '<a href="'.$gourl.'" target="_blank">'.$classname.'</a><br>';
