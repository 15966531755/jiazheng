...
$row_a = $dosql->GetOne("SELECT * FROM `area` WHERE `areaid`=".$row['area']);	
		...
		for($i=0; $i<$flagnum; $i++)
		{
			...
			<td class="action endCol"> <span><a href="goodsksyy_update.php?id=<?php echo $row['id']; ?>">编辑</a></span> |<span class="nb"><a href="goodsksyy_save.php?action=del&id=<?php echo $row['id']; ?>" onclick="return ConfDel(0)">删除</a></span></td>
		</tr>
		<?php
		}	
	?>
