...
function AddUserFavorite(aid,cid,tid)
{
	//var aid   = $("#aid").val();
	var aid   = aid;
	var cid   = cid;
	...
	$.ajax({
		url : "member.php",
		type: "post",
		data:{"a":"savefavorite", "aid":aid,"cid":cid,"tid":tid, "molds":molds},
		dataType:'html',
		success:function(data){
			if(data == 1){
				alert("收藏成功！");
...
function AddUserFavorites(aid)
{
	...
	$.ajax({
		url : "member.php",
		type: "post",
		data:{"a":"savefavorites", "aid":aid,"molds":molds},
		dataType:'html',
		success:function(data){
			if(data == 1){
				alert("收藏成功！");
