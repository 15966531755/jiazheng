...
function get_app_uid($app = 'renren') {
    ...
    if ($app == 'renren') {
        ...
        $uid = isset($_SESSION['weixin_token']['uid']) ? $_SESSION['weixin_token']['uid'] : 0;
    }
...
function get_app_info($app = 'renren') {
    ...
    if ($app == 'renren') {
        ...
        $weixinclient = new weixinclient($_SESSION['weixin_token']['access_token'], $_SESSION['weixin_token']['openid']);
        $result = $weixinclient->get_user_info();
		
		//var_dump($result);
        $result['name'] = $result['nickname'];
        $result['uid'] = $_SESSION['weixin_token']['openid'];
        $result['face'] = $result['headimgurl'];
        $result['gender'] = $result['sex'] == 1 ? '男' : '女';
        $result['birthday'] = '';
    }
...
function get_app_name($app = 'renren') {
    $app_array = array('renren' => '人人网', 'weibo' => '新浪微博', 'qq' => '腾讯QQ', 'weixin' => '腾讯微信');
