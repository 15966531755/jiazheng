<?php	require_once('weixinclient.php');
...
class weixin extends weixinOAuth {
    ...
    function verify() {
        if (isset($_SESSION['weixin_token']) && $_SESSION['weixin_token'] && isset($_SESSION['weixin_token']['uid'])) {
            return true;
        } else {
            return false;
        }
...
class weixinOAuth {
    ...
    public $http_info;
    public $useragent = 'weixin OAuth2.0';
    public $debug = FALSE;
    ...
    function openidURL() {
        return "https://api.weixin.qq.com/sns/oauth2/me";
    }

    function __construct($access_token = NULL, $refresh_token = NULL) {
        $this->app_id = WEIXIN_AKEY;
        $this->app_secret = WEIXIN_SKEY;
    ...
    function getAccessToken($type = 'code', $keys = array()) {
		...
		$response = str_replace('"','',$response);
	$response = str_replace(',','&',$response);
//		  $response = $this->filterResponse("https://api.weixin.qq.com/sns/oauth2/access_token?appid=".$this->app_id."&secret=".$this->app_secret."&code=".$keys['code']."&grant_type=authorization_code");
	
		
        parse_str($response, $token);
	 //  var_dump($token) ;
//		throw new Exception($token['access_token']);
...
//			
//        $result =  array(str_replace(':','=>',$response));;
    ...
    function http($url, $method, $postfields = NULL, $headers = array()) {
       ...
       // curl_setopt($ci, CURLOPT_SSL_VERIFYHOST, 1);  //出现问题  
