...
class Connect
{
    ...
    function weixin_token()
	{
        ...
        $url = $this->weixin->getAuthorizeURL($callback, 'code', $state, WEIXIN_SCOPE);
    ...
    function weixin_callback()
	{
        ...
        if (!isset($_SESSION['weixin_state']) || ($state != $_SESSION['weixin_state']))
	//	echo  $state;
//		echo "sssssssss";
//		echo $_SESSION['weixin_state'];
            exit('参数错误！请返回重新使用微信登录！');
	
        $keys = array('code' => $code, 'redirect_uri' => "{$this->url}?method=weixin_callback");
        $token = $this->weixin->getAccessToken('code', $keys);

        if(isset($token['error']))
            exit('验证错误！请返回重新使用微信登录！');

        $_SESSION['weixin_token'] = $token;
        $openid = $token['openid'];
        $_SESSION['weixin_token'] = array('access_token' => $token['access_token'], 'openid' => $openid, 'uid' => $openid, 'expires_in' => $token['expires_in']);

        go("connect.php?method=callback&app=weixin");
    }
