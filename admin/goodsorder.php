...
<body>
<div class="topToolbar"> <span class="title">服务订单管理</span> <a href="javascript:location.reload();" class="reload">刷新</a></div>
...
<? 
	$rows = $dosql->GetOne("SELECT levelname FROM `#@__admin` WHERE `username`= '".$_SESSION['admin']."'");
...
?>
	<table width="100%" border="0" cellpadding="0" cellspacing="0" class="dataTable">
		...
		if($rows['levelname'] == 1){
		$sql = "SELECT * FROM `#@__yuyue`";
		}else{
			$sql = "SELECT * FROM `#@__yuyue` where  jingliren='".$_SESSION['admin']."'";
			}
...
$row_a = $dosql->GetOne("SELECT * FROM `area` WHERE `areaid`=".$row['area']);
		...
		<tr align="left" class="dataTr">
			...
			<td class="number"><?php echo $row['date']; ?></td>
            <td><?php echo $row['ewxq']; ?></td>
            <td><?php echo $row['jingliren']; ?></td>
			...
			<td class="action endCol"><a href="goodsorder_update.php?id=<?php echo $row['id']; ?>">修改</a></span> | <span class="nb"><a href="goodsorder_save.php?action=del2&id=<?php echo $row['id']; ?>" onclick="return ConfDel(0)">删除</a></span></td>
		<? }?>
        </tr>
		<?php
		}	
